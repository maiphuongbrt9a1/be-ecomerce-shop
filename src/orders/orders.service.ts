...
export class OrdersService {
  ...
  async getAllOrdersWithDetailInformation(
    ...
    try {
      ...
      // Status filter accepts either a raw OrderStatus value (one tab per
      // status on the FE) or the special 'pending_return' key that maps to
      // a cross-cutting request filter. 'all' / empty / unknown → no filter.
      ...
      if (statusFilter === 'pending_return') {
        ...
        statusFilter &&
        (Object.values(OrderStatus) as string[]).includes(statusFilter)
      ) {
        tabWhere.status = statusFilter as OrderStatus;
      }
